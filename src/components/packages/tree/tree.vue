<template> 
    <div class="el-checkbox__label-hide">   
        <table width="100%" cellpadding="0" cellspacing="0" class="table5">
            <thead>  
                 <tr><td :style="{paddingLeft:left + 'px'}" class="left">{{sub.name}} </td> <td ><el-checkbox  :indeterminate="isIndeterminate" :disabled="disabled" @change="subChange" v-model="checkSubAll"></el-checkbox></td></tr> 
            </thead>
        </table>
        <el-checkbox-group v-model="checkChildAll"  @change="childrenChange"> 
            <table width="100%" cellpadding="0" cellspacing="0" class="table5">
                <tbody>  
                    <tr v-for="children in sub.subPermissions" :key ="children.code"><td style="padding-left:90px" class="left">{{children.name}}</td><td> <el-checkbox  :disabled="disabled"  
                    :label='children.code'></el-checkbox></td></tr> 
