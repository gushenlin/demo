...
const user = {
  ...
  mutations: {
    [USER_SIGNIN] (state, userLoginData) {
      window.sessionStorage.setItem('CURR_USER_PRESCRIPTION_SUPERVISE', JSON.stringify(userLoginData));
      state.userLogin = userLoginData.userLogin;
      state.title = userLoginData.title;
    },
    [USER_SIGNOUT] (state) {
    ...
    saveMenuList (state, payload) {
      window.sessionStorage.setItem('MENULIST', JSON.stringify(payload.paras));
