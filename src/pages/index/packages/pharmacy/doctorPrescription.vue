<template>
  <div class="cloud-component doctorPrescription">
        ...
        <el-form-item label="查询日期：">
          ...
          <el-form-item label="选择医院：" class="searchHospital" prop="hospitalId">
            <el-select v-model.trim="ruleForm.hospitalId" placeholder="请选择医院" :clearable="true">
                <el-option v-for="(item,index) in departmentList"  :key="item.id" :label="item.hospitalName" :value="item.id"></el-option>
            </el-select>
          </el-form-item>
        </el-form-item>
    ...
    <div class="cloud-search-list">
      ...
      <echarts-contain containType="big" :parentHeight="routerViewHeight" :heightRatio="1">
        <el-table :height="(dataList.content && dataList.content.length>0)?(routerViewHeight*1-40):(routerViewHeight*1-5)"
          ...
          <el-table-column prop="hospitalName" label="医院名称" min-width="18%" show-overflow-tooltip>
          </el-table-column>
    ...
    methods: {
      ...
      getDataList (pageNo = this.pageNo, pageSize = this.pageSize) {
        ...
        that.$axios({
          ...
          url: '/peps/presc/drugstroe/statistics/doctor?v=' + (+new Date()),
          data: formData
        }).then((response) => {
          that.loading = false;
          if (response.data.code == 0) {
